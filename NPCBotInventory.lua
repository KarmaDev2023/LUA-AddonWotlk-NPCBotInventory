...
itemLinkFrame:SetSize(350, 570 + 40)
...
-- Global variables to track the last frame position
local lastFrameXOffset = 0
local lastFrameYOffset = 0
local frameOffsetIncrement = 30 -- Pixels to offset each new frame

local function CloseAllBotFrames()
    for botName, frame in pairs(botFrames) do
        frame:Hide()
...
local function ShowSavedInventory(botName)
    ...
    if not botFrames[botName] then
        ...
        local frameX, frameY = itemLinkFrame:GetCenter()
        frame:SetPoint("CENTER", UIParent, "BOTTOMLEFT", frameX + lastFrameXOffset, frameY + lastFrameYOffset)

        -- Update the last frame position for the next frame
        lastFrameXOffset = lastFrameXOffset + frameOffsetIncrement
        lastFrameYOffset = lastFrameYOffset + frameOffsetIncrement

        -- Reset the offset if it exceeds a certain threshold to avoid going offscreen
        if lastFrameXOffset > 150 or lastFrameYOffset > 150 then
            lastFrameXOffset = 0
            lastFrameYOffset = 0
        end
    end
    ...
    for i, link in ipairs(botInventories[botName]) do
        ...
        -- Tooltip functionality for the item name text
        local tooltipButton = CreateFrame("Button", nil, frame)
        tooltipButton:SetAllPoints(itemNameText)
        tooltipButton:EnableMouse(true)
        tooltipButton:SetScript("OnEnter", function()
            GameTooltip:SetOwner(tooltipButton, "ANCHOR_RIGHT")
            GameTooltip:SetHyperlink(link)
            GameTooltip:Show()
        end)
...
StaticPopupDialogs["CLEAR_ALL_BOTS_CONFIRMATION"] = {
    text = "Are you sure you want to clear all bot inventory data for this character?",
    button1 = "Yes",
    button2 = "No",
    OnAccept = function()
        local playerName = UnitName("player") -- Get the current player's name
...
clearAllButton:SetSize(135, 30)
clearAllButton:SetPoint("BOTTOM", botListFrame, "BOTTOM", 0, 10)
clearAllButton:SetText("Clear All Bot Data")

-- Set the button's vertex color to black
local nt = clearAllButton:GetNormalTexture()
if nt then
    nt:SetVertexColor(1, 1, 1)  -- RGB for Black: 0, 0, 0
end

local fontString = clearAllButton:GetFontString()
if fontString then
    fontString:SetTextColor(1, 1, 1)  -- RGB for White: 1, 1, 1 
end
...
local function OnChatMessage(self, event, message, sender, ...)
    if (event == "CHAT_MSG_MONSTER_WHISPER") then
        ...
        -- Check if the message contains "Rezzing You"
        if string.find(message, "Rezzing You") then
            print("Skipping bot:", sender, "due to 'Rezzing You' message.")
        ...
        if (link) then
            ...
            -- Set up tooltip for item name text
            local tooltipButton = CreateFrame("Button", nil, itemLinkFrame)
            tooltipButton:SetAllPoints(itemNameText)
            tooltipButton:EnableMouse(true)
            tooltipButton:SetScript("OnEnter", function()
                GameTooltip:SetOwner(tooltipButton, "ANCHOR_RIGHT")
